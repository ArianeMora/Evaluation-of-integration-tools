...
message("Making datasets for CNV, RNAseq, and DNA methylation of ", data_name, " in folder:", data_folder)
phenotype <- read.csv(gzfile(file.path(data_folder, paste0("TCGA-", data_name, ".GDC_phenotype.tsv"))),  header = T, sep = "\t")
...
phenotype$sample_id <- paste(gsub(" ", "", phenotype$sample_type.samples, fixed = TRUE), phenotype$submitter_id.samples, sep="_")
...
## Annotate the methylation CpGs to gene names (using ensembl gene ID)
mapping <- read.csv('illuminaMethyl450_hg38_GDC_2500_promoter_mapping.csv')
mapping <- mapping[!is.na(mapping$external_gene_name), ]
# If there are CpGs that map to multiple genes only pick the gene that it is closest to
mapping$dist_to_gene <- (mapping$start_position - mapping$chromStart)
mapping$dist_to_gene[mapping$dist_to_gene < 0] <- (mapping$chromEnd - mapping$start_position) # i.e. it is reverse stranded
mapping <- mapping[order(mapping$id, mapping$dist_to_gene), ]
mapping <- mapping[ !duplicated(mapping$id), ]

mapping <- mapping[!duplicated(mapping$id),] # Remove any duplicated CpGs (i.e. this is where a CpG mapped to multiple genes)
rownames(mapping) <- mapping$id
me.exp <- me.exp_all
# map between the two datasets
annotated_cpgs <- intersect(rownames(me.exp), mapping$id)
...
cols <- intersect(phenotype$submitter_id_safe, colnames(cn))
...
samples <- samples[a1, ]
message("After merging your sample distribution is now:")
# Print out the number of normal and tumour samples
table(samples$sample_type.samples)
...
colnames(ge.exp2) <- samples$sample_id
colnames(cn2) <- samples$sample_id
colnames(me.exp2) <- samples$sample_id
